...
// NORDIC: Declare global adc sample variable
volatile uint32_t battery_level;
bool do_battery_level_measurement = false;
...
static void timer_timeout_handler_sensor(void * p_context)
{
    ...
    // OUR_JOB: Update our dummy data and send to characteristic
...
{
    // OUR_JOB: Start our timer
    uint32_t err_code;
    err_code = app_timer_start(m_app_timer_battery_id, TIMER_INTERVAL_BATTERY, NULL);
    APP_ERROR_CHECK(err_code);
    
    err_code = app_timer_start(m_app_timer_sensor_id, TIMER_INTERVAL_SENSOR, NULL);
    APP_ERROR_CHECK(err_code);
}
